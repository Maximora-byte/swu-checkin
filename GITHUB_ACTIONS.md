...
- ❌ **实际执行时间严重不可控**：定时任务可能延迟几分钟、几十分钟，甚至数小时才执行
- ❌ **高峰期更严重**：在 UTC 整点附近（北京时间 8:00/16:00/0:00 等）延迟尤其明显
- ❌ **免费账户优先级低**：GitHub 会优先调度付费用户的任务，免费用户排队时间更长
- ❌ **无法保证准时签到**：如果学校签到时间窗口较短（如 21:00-23:30），可能因延迟错过

**建议：**
- 如果你对签到时间有严格要求，建议使用云服务器定时任务（如阿里云、腾讯云）或本地部署
- GitHub Actions 更适合作为备用方案，或者签到时间窗口较宽松的场景
- 本项目设置了两个执行时间（21:15、21:45），第二次执行可作为第一次延迟的补偿

---

## 功能说明

- **自动签到**: 每天北京时间 21:15、21:45 自动执行签到
...
### 自动运行
配置完 Secrets 后，Actions 会在每天北京时间 21:15 和 21:45 自动执行。

⚠️ **注意：GitHub Actions 定时任务存在严重延迟问题**
- 实际执行时间可能晚于设定时间数分钟、数十分钟，甚至数小时
- 免费账户在高峰期延迟尤其严重，可能完全错过签到时间窗口
- 第二次执行（21:45）可作为第一次延迟的补偿，但仍不能保证准时
- 如果多次因延迟错过签到，建议改用云服务器或本地定时任务
...
schedule:
  # cron 格式: 分 时 日 月 周
  # 北京时间 = UTC + 8
  # ⚠️ GitHub Actions 定时任务存在严重延迟问题，实际执行时间可能晚数小时
  - cron: '15 13 * * *'  # 北京时间 21:15（实际执行时间不可控）
  - cron: '45 13 * * *'  # 北京时间 21:45（实际执行时间不可控）
```
