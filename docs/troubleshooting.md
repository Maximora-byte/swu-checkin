...
提交阶段的诊断采用固定分类，不记录服务端响应正文、message 或未知业务码原值：

| 日志分类 | 含义 | 建议 |
| --- | --- | --- |
| `请求超时` | 提交请求超时，服务端结果不明确 | 等待回读和下一次独立定时任务，不要立即连续手动重跑 |
| `连接异常` | 连接在确认响应前中断 | 同上，先查询状态 |
| `HTTP 503` | 服务端返回该 HTTP 状态 | 等待学校服务恢复；日志只保留状态码 |
| `业务码已返回` | 响应含非成功 `code/status` | 原值有意不记录；运行 `doctor` / `probe` 辅助区分接口问题 |
| `显式失败标志` | 响应明确给出失败布尔/结果标志 | 运行只读诊断，不修改 payload |
| `响应结构异常` | 响应不是可识别对象 | 可能是接口变更，提交脱敏结构信息 |

这些分类不会改变提交安全语义。POST 发生后，无论是 timeout、连接异常、HTTP 错误还是业务失败，当前进程都只执行只读回查，不通过 outer retry 再次 POST。业务码可能实际承载 token、session 或账号标识，因此即使看起来像短数字或普通英文，也不会被记录、哈希或部分遮罩。
